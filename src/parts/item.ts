...
export class Item extends MyDisplay {
  ...
  constructor(opt:any) {
    ...
    this._radius = Util.instance.map(this._itemId, baseRadius * 0.1, baseRadius, 0, Conf.instance.ITEM_NUM);

    // 円周に収まる数だけつくる
    const dist = this._radius * 2 * 3.14;
    // const num = ~~(dist / (this._fontSize * 0.65));
    const num = ~~(dist / (this._fontSize * 0.75));
  ...
  protected _update(): void {
    ...
    const size = Func.instance.sw() / Conf.instance.LINE_NUM;

    const mx = Mouse.instance.easeNormal.x
    const my = Mouse.instance.easeNormal.y

    let radius = this._radius * Util.instance.map(mx, 1, 1.25, -1, 1);
